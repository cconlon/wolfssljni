...
public class WolfSSLCRLTest {
     ...
     */
    private WolfSSLCRL loadTestCRLFromFile(int format) throws WolfSSLException {
    ...
    public void testSign_NativePemKeyBytesRegression()
        ...
        try {
            ...
            byte[] sig = crl.getSignature();
            assertNotNull("signature should be available after native PEM sign",
                sig);
            assertTrue("signature length should be > 0", sig.length > 0);
        }
        finally {
    ...
    public void testLoadFromDer()
        ...
        WolfSSLCRL loadedCrl2 = new WolfSSLCRL(der, WolfSSL.SSL_FILETYPE_ASN1);
    ...
    public void testLoadFromPem()
        ...
        WolfSSLCRL loadedCrl = new WolfSSLCRL(pem, WolfSSL.SSL_FILETYPE_PEM);
        assertNotNull(loadedCrl);
    ...
    public void testLoadFromFile()
        ...
        WolfSSLCRL derCrl = loadTestCRLFromFile(WolfSSL.SSL_FILETYPE_ASN1);
        assertNotNull(derCrl);
        derCrl.free();

        /* Test null path throws exception */
        try {
            new WolfSSLCRL((String)null, WolfSSL.SSL_FILETYPE_PEM);
            fail("null path should throw exception");
        } catch (WolfSSLException e) {
            /* expected */
        }
        ...
        if (WolfSSL.CrlGenerationEnabled()) {
            ...
            File tempDer = File.createTempFile("test_crl_load_", ".der");
            genCrl.writeToFile(tempDer.getAbsolutePath(),
                WolfSSL.SSL_FILETYPE_ASN1);

            WolfSSLCRL loadedCrl = new WolfSSLCRL(tempDer.getAbsolutePath(),
                WolfSSL.SSL_FILETYPE_ASN1);
            assertNotNull(loadedCrl);
            assertEquals(genCrl.getVersion(), loadedCrl.getVersion());
    ...
    public void testGetIssuerName()
        ...
        assertNotNull("File CRL issuer should not be null", fileName);
        ...
        if (WolfSSL.CrlGenerationEnabled()) {
            ...
            assertNotNull("Issuer name should not be null", name);
            assertTrue("Issuer should contain wolfSSL",
