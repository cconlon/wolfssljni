...
public class WolfSSLTest {
    ...
    public void test_getNamedGroupFromString_PQC() {
        ...
        /* ML-KEM standalone: both compact and JEP 527 spellings */
        ...
        /* IETF hybrids: JEP 527 mixed-case and all-caps */
    ...
    public void test_PQC_FeatureDetect_NativeReturns() {
        ...
        boolean pqc    = WolfSSL.PQCEnabled();
        boolean mlkem  = WolfSSL.MLKEMEnabled();
        boolean mldsa  = WolfSSL.MLDSAEnabled();
        boolean oldIds = WolfSSL.MLKEMOldIdsEnabled();

        if (mlkem) {
            assertTrue("PQCEnabled() must be true when MLKEMEnabled() is " +
                "(HAVE_PQC is auto-defined by WOLFSSL_HAVE_MLKEM)", pqc);
        }

        /* ML-DSA and the legacy-IDs flag are independent of MLKEM gating.
         * The only invariant we can assert is that native calls returned
         * without crashing. Reference variables to silence "unused" warnings
         * on older toolchains. */
        if (mldsa || oldIds) {
            /* nothing */
        }
