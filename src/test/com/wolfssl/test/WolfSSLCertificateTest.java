...
public class WolfSSLCertificateTest {
    ...
    public void testWolfSSLCertificateAuthorityKeyIdRoundtrip()
        ...
        /* Check if the native encoder did not wrap the raw keyId in
         * SEQUENCE { [0] keyId } per RFC 5280 4.2.1.1, skip rather than fail.
         * The encoder bug lives in wolfSSL prior to PR #10370, not wolfssljni.
         * We want the test here to run for wolfSSL versions that are fixed,
         * but not for broken versions. */
        ...
        /* setAuthorityKeyIdEx(WolfSSLCertificate issuer) round trip test */
    ...
    /* Returns true iff the AKID extension on this DER cert decodes as a well
     * formed SEQUENCE { [0] keyId } per RFC 5280 4.2.1.1. Returns false if the
     * extension is missing, malformed, or the inner content begins with
     * anything other than the SEQUENCE tag (which is the unfixed encoder
     * symptom: raw key id bytes written directly into the OCTET STRING). Used
     * by testWolfSSLCertificateAuthorityKeyIdRoundtrip() to skip on native
     ...
     *     04 LL <inner-bytes>
     *
     * For a properly formed AKID, <inner-bytes> starts with 0x30 (SEQUENCE);
     * for an unfixed encoder it is a raw 20-byte keyId starting with whatever
     * happens to be byte 0 of the keyId. */
    private boolean akidExtensionWellFormed(byte[] derCert)
        ...
        /* Skip outer OCTET STRING wrapper. Short-form length only, since real
         * AKID extensions fit in <128 bytes. */
        ...
        /* Inner must start with SEQUENCE tag (0x30) for a properly formed
         * AuthorityKeyIdentifier. */
        return (extWrapped[2] & 0xFF) == 0x30;
    }

    /* Build, populate, AKID-stamp, and sign a cert. Returns the DER bytes or
     * null if the AKID setter returned NOT_COMPILED_IN at runtime (native
     * wolfSSL pre PR 10370). Exactly one of {akidRaw, issuerForEx} must be non
     * null to select which AKID setter to exercise. */
    ...
    /* Parse DER cert with JDK CertificateFactory and assert the AKID extension
     * (OID 2.5.29.35) decodes as a properly formed SEQUENCE {
