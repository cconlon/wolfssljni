...
class WolfSSLPQCTestUtil {
    ...
    /**
     * Returns true if native wolfSSL can complete an ML-DSA TLS 1.3 cert-auth
     * handshake via wolfJSSE. Drives test SKIP decisions for ML-DSA cert-auth
     * tests so they pass cleanly on older native wolfSSL.
     *
     * wolfJSSE's ML-DSA cert-auth path needs native wolfSSL PR #10310, which
     * added ML-DSA SPKI / PKCS#8 DER support to d2i_PUBKEY / d2i_PrivateKey.
     * That PR landed after the 5.9.1 release tag, so 5.9.1 (and earlier) fail
     * the cert-auth handshake while master and post-5.9.1 stable releases
     * succeed.
     *
     * Buffer-level probes (e.g. ctx.usePrivateKeyBuffer alone) do not detect
     * the gap because some failures only show up during the handshake verify
     * pass. So this check does a real ML-DSA-44 SSLEngine handshake, which
     * exercises the code paths the gated tests will exercise.
     */
    static synchronized boolean nativeMlDsaCertParsingSupported() {
        ...
        if (!WolfSSL.MLDSAEnabled() || !WolfSSL.TLSv13Enabled()) {
            nativeMlDsaCertParsingSupported = Boolean.FALSE;
            return false;
        }

        String serverCert = PQC_CERT_DIR + "/server-mldsa44.pem";
        String serverKey  = PQC_CERT_DIR + "/server-mldsa44-priv.pem";
        String rootCert   = PQC_CERT_DIR + "/root-mldsa44.pem";

        if (!pqcCertExists(serverCert) || !pqcCertExists(serverKey) ||
            !pqcCertExists(rootCert)) {
            /* Certs missing. Treat as unsupported so tests are skipped. */
            nativeMlDsaCertParsingSupported = Boolean.FALSE;
            return false;
        }

        try {
            KeyManager[] km = keyManagerFromPem("server-mldsa44",
                serverCert, serverKey, "ML-DSA-44");
            TrustManager[] tm = trustManagerFromPem(rootCert);

            WolfSSLTestFactory tf = new WolfSSLTestFactory();
            SSLContext serverCtx = tf.createSSLContext("TLSv1.3", "wolfJSSE",
                null, km);
            SSLContext clientCtx = tf.createSSLContext("TLSv1.3", "wolfJSSE",
                tm, null);
            ...
            SSLEngine client = clientCtx.createSSLEngine(
                "wolfSSL ML-DSA probe", 11111);
            client.setUseClientMode(true);

            int ret = tf.testConnection(server, client, null,
                new String[] { "TLSv1.3" }, "probe");
            nativeMlDsaCertParsingSupported =
                (ret == 0) ? Boolean.TRUE : Boolean.FALSE;
    ...
    static final String MLDSA_CERT_PARSING_SKIP_MSG =
        "Native wolfSSL lacks ML-DSA TLS 1.3 cert-auth support " +
