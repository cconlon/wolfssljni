...
public class WolfSSLPQCAuthKeyStoreTest {
    ...
    private void runHandshake(int level, String storeType, String ext)
        ...
        /* Server keystore: private key + CA-signed cert chain. The store
         * password matches WolfSSLTestFactory.jksPass, which the factory
         * also uses to recover private keys in createKeyManager(). */
        KeyStore serverKs = KeyStore.getInstance(storeType);
        FileInputStream in = new FileInputStream(serverPath);
        try {
            serverKs.load(in, WolfSSLTestFactory.jksPass);
        } finally {
            in.close();
        }
        KeyManager[] km = tf.createKeyManager("SunX509", serverKs, PROVIDER);
        ...
        try {
            caKs.load(in, WolfSSLTestFactory.jksPass);
        } finally {
            in.close();
        }
        TrustManager[] tm = tf.createTrustManager("SunX509", caKs, PROVIDER);
