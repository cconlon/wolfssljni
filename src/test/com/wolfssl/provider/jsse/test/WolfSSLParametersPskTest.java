...
public class WolfSSLParametersPskTest {
    ...
    public static void testSetup() throws WolfSSLException {
        ...
        /* Find an available PSK cipher suite, preferring
         * ephemeral (ECDHE > DHE) over static PSK */
        try {
            SSLContext ctx = SSLContext.getInstance(
                "TLSv1.2", engineProvider);
            ctx.init(null, null, null);
            String[] suites = ctx.createSSLEngine()
            ...
            if (ecdhe != null) { pskCipher = ecdhe; }
            else if (dhe != null) { pskCipher = dhe; }
            else { pskCipher = plain; }
        } catch (Exception e) {
            /* ignore */
        }
        Assume.assumeTrue(
            "No PSK cipher suite available",
            pskCipher != null);
        System.out.println("\tUsing PSK cipher: " + pskCipher);
    }
