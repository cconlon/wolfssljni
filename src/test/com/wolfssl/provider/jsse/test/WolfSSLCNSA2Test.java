...
public class WolfSSLCNSA2Test {
    ...
    public void testCNSA2_FullStack() throws Exception {
        ...
        synchronized (WolfSSLPQCTestUtil.GROUP_PROP_LOCK) {
             ...
             * level). With no classical fallback in the list, a successful
             * handshake proves PQC kex actually happened. */
            String prevCurves =
                WolfSSLPQCTestUtil.setCurvesProperty("SECP384R1MLKEM1024");
            try {
                ...
                WolfSSLPQCTestUtil.restoreCurvesProperty(prevCurves);
            }
        }
    }
    ...
    public void testCNSA2_NoFallbackToTls12() throws Exception {
        ...
        synchronized (WolfSSLPQCTestUtil.GROUP_PROP_LOCK) {
            /* Include a classical group alongside the PQC hybrid. wolfJSSE
             * fails closed when a configured group list is left empty after
             * PQC filtering on a non-TLS-1.3 session, so an all-PQC list
             * would make the TLS 1.2-only client fail during handshake setup
             * before the ClientHello is even sent. The classical entry keeps
             * the ClientHello well formed so this test exercises protocol
             * version negotiation, not group configuration failure. */
            String prevCurves = WolfSSLPQCTestUtil.setCurvesProperty(
                "SECP384R1MLKEM1024,secp384r1");
            try {
                KeyManager[] serverKm = WolfSSLPQCTestUtil.keyManagerFromPem(
                    "server-mldsa87", SERVER_CERT, SERVER_KEY, "ML-DSA-87");
                TrustManager[] clientTm =
