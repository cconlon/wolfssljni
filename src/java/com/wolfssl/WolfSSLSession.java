...
public class WolfSSLSession {
    ...
    /**
     * Sets the TLS supported groups (Supported Curves extension) to be used
     * for the session, using native wolfSSL named-group enum values.
     * Multiple groups are appended to the extension via calls into native
     * wolfSSL_UseSupportedCurve(). Iteration continues across all entries
     * even after one fails. Groups that succeed are added, groups that
     * fail are not.
     *
    ...
    public int useSupportedCurves(int[] groups)
        ...
        synchronized (sslLock) {
            ...
            for (int group : groups) {
                ret = useSupportedCurve(this.sslPtr, group);
