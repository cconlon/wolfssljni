...
public class WolfSSLCertificate implements Serializable {
     ...
     * used when generating X509v3 certificates.
     *
     * With native wolfSSL with the AKID encoder fix in PR #10370, the bytes are
     * wrapped in SEQUENCE { [0] keyIdentifier } per RFC 5280 4.2.1.1. Older
     * native wolfSSL builds without the fix emit a malformed AKID extension
     * that parsers reject.
     *
     * @param akid Raw key identifier bytes (ex: the SHA-1 hash of the issuer
     *        public key, or the issuer SubjectKeyIdentifier value). Bytes
     *        must not be a pre-encoded AuthorityKeyIdentifier SEQUENCE.
     *
     * @throws IllegalStateException if WolfSSLCertificate has been freed.
     * @throws WolfSSLException if invalid arguments or native JNI error occurs.
     */
