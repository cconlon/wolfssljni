...
#
# Output is always written to examples/certs/pqc/ (the script's own
# directory + /pqc), regardless of the current working directory. Each
# level produces:
#
...
#   ./examples/certs/gen-mldsa-certs.sh
#
...
# adds ML-DSA chain generation, this script can be retired in favor of
# update-certs.sh pulls.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTDIR="$SCRIPT_DIR/pqc"
mkdir -p "$OUTDIR"
